...
5. Ensure these helper entities exist, or adjust `pyscript/apps/temptamer/config.py` to match your setup:
   ...
   - `input_number.temptamer_setpoint_adjustment`, with your preferred minimum and maximum and step `0.1`, to offset every zone together. The controller does not clamp this manual offset; automatic comfort scores remain clamped to `-3` through `3`. Effective targets are `base - (clamp(auto) - global)`.
...
- Each comfort-score helper shifts its zone's heat and cool `enable_outside`, `continue_until`, and `ideal_target` thresholds by **subtracting** the helper value. A positive warmth score therefore lowers thresholds and delays heat, while a negative cold/dry score raises thresholds and requests heat sooner. `input_number.temptamer_setpoint_adjustment` remains a conventional positive-is-warmer offset for every zone. The offsets compose, keep threshold spacing intact, and apply to zone opening, equipment demand, fan logic, dispatch planning, and idle-demand forecasts.
